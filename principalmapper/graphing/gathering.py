...
def create_graph(
    ...
    try:
        ...
        policies_result.extend(
            get_sns_topic_policies(
                session, region_allow_list, region_deny_list, client_args_map
            )
        )
        policies_result.extend(
...
def get_nodes_groups_and_policies(iamclient) -> dict:
    ...
    for p in policy_results:
        ...
        doc = [x["Document"] for x in p["PolicyVersionList"] if x["IsDefaultVersion"]][
            0
        ]
    ...
    for g in group_results:
        ...
        if "GroupPolicyList" in g:  # have to key-check these
            ...
            result[
                "policies"
            ] += group_policies  # this is just adding the inline policies for the group

        for p in g["AttachedManagedPolicies"]:
            group_policies.append(
                _get_policy_by_arn_or_raise(p["PolicyArn"], result["policies"])
            )
    ...
    for u in user_results:
            ...
            for p in u["UserPolicyList"]:
                user_policies.append(
                    Policy(
                        u[
                            "Arn"
                        ],  # inline policies inherit the Arn of their principal for the purposes of tracking
                        p["PolicyName"],
                        p["PolicyDocument"],
                    )
                )
        ...
        if "PermissionsBoundary" in u:
            boundary_policy = _get_policy_by_arn_or_raise(
                u["PermissionsBoundary"]["PermissionsBoundaryArn"], result["policies"]
            )
        else:
    ...
    for r in role_results:
        ...
        result["nodes"].append(
            Node(
                ...
                [x["Arn"] for x in r["InstanceProfileList"]],
                0,
                False,
                False,
                None,
        ...
        if arns.get_resource(node.arn).startswith("user/"):
            ...
            try:
                ...
                if "AccessDenied" in str(ex):
                    logger.warning(
                        f"Access denied for listing access keys for user: {user_name}"
                    )
                    node.access_keys = None
                else:
                    raise ex
...
def get_s3_bucket_policies(
    session: botocore.session.Session, client_args_map: Optional[dict] = None
) -> List[Policy]:
    ...
    for bucket in buckets:
        ...
        try:
            ...
            if "NoSuchBucketPolicy" in str(ex):
                logger.info(
                    'Bucket {} does not have a bucket policy, adding a "stub" policy instead.'.format(
                        bucket
                    )
                )
                result.append(
                    Policy(
                        bucket_arn, bucket, {"Statement": [], "Version": "2012-10-17"}
                    )
                )
            else:
                logger.info(
                    "Unable to retrieve bucket policy for {}. You should add this manually. Continuing.".format(
                        bucket
                    )
                )
...
def get_kms_key_policies(
    ...
    for kms_region in get_regions_to_search(
        session, "kms", region_allow_list, region_deny_list
    ):
        try:
            # Grab the keys
            cmks = []
            ...
            for cmk in cmks:
                policy_str = kmsclient.get_key_policy(KeyId=cmk, PolicyName="default")[
                    "Policy"
                ]
                result.append(
                    Policy(
                        ...
                        cmk.split("/")[
                            -1
                        ],  # CMK ARN Format: arn:<partition>:kms:<region>:<account>:key/<Key ID>
...
def get_sns_topic_policies(
    ...
    for sns_region in get_regions_to_search(
        session, "sns", region_allow_list, region_deny_list
    ):
        try:
            # Grab the topics
            topics = []
            ...
            for topic in topics:
                policy_str = snsclient.get_topic_attributes(TopicArn=topic)[
                    "Attributes"
                ]["Policy"]
                result.append(
                    Policy(
                        ...
                        topic.split(":")[
                            -1
                        ],  # SNS Topic ARN Format: arn:<partition>:sns:<region>:<account>:<Topic Name>
                        json.loads(policy_str),
                    )
                )
...
def get_sqs_queue_policies(
    ...
    for sqs_region in get_regions_to_search(
        session, "sqs", region_allow_list, region_deny_list
    ):
        try:
            ...
            for queue_url in queue_urls:
                ...
                if "Policy" in sqs_policy_response:
                    ...
                    logger.info(
                        'Queue {} does not have a queue policy, adding a "stub" policy instead.'.format(
                            queue_name
                        )
                    )
        except botocore.exceptions.ClientError as ex:
...
def get_secrets_manager_policies(
    ...
    for sm_region in get_regions_to_search(
        session, "secretsmanager", region_allow_list, region_deny_list
    ):
        try:
            # Grab the ARNs of the secrets in this region
            secret_arns = []
            smclient = session.create_client(
                "secretsmanager", region_name=sm_region, **smargs
            )
            ...
            for page in list_secrets_paginator.paginate():
                if "SecretList" in page:
                    for entry in page["SecretList"]:
                        if (
                            "PrimaryRegion" in entry
                            and entry["PrimaryRegion"] != sm_region
                        ):
            ...
            for secret_arn in secret_arns:
                ...
                if "ResourcePolicy" in sm_response and sm_response["ResourcePolicy"]:
                    ...
                    logger.info(
                        "Storing the resource policy for secret {}".format(secret_arn)
                    )
                else:
                    result.append(
                        Policy(
                    ...
                    logger.info(
                        'Secret {} does not have a resource policy, inserting a "stub" policy instead'.format(
                            secret_arn
                        )
                    )
...
def get_policies_and_fill_out(
    iamclient, nodes: List[Node], groups: List[Group]
) -> List[Policy]:
    ...
    for node in nodes:
        ...
        logger.debug(
            "perm boundary of {}: {}".format(
                node.searchable_name(), node.permissions_boundary
            )
        )
        if node.permissions_boundary is not None and isinstance(
            node.permissions_boundary, str
        ):
            logger.debug(
                "Getting boundary policy: {}".format(node.permissions_boundary)
            )
            # reduce API calls, search existing policies for matching ARNs
            ...
            if policy_object is None:
                ...
                logger.debug("Policy cache miss, calling API")
                policy_response = iamclient.get_policy(
    ...
    for group in groups:
        group_name = arns.get_resource(group.arn).split("/")[
            -1
        ]  # split by slashes and take the final item
        ...
        for policy_name in inline_policies["PolicyNames"]:
            logger.debug("Grabbing inline policy: {}".format(policy_name))
            inline_policy = iamclient.get_group_policy(
...
def update_admin_status(
    nodes: List[Node], scps: Optional[List[List[dict]]] = None
) -> None:
    ...
    for node in nodes:
        ...
        if query_interface.local_check_authorization_handling_mfa(
            node, action, node.arn, {}, service_control_policy_groups=scps
        )[0]:
            node.is_admin = True
            continue
        ...
        if query_interface.local_check_authorization_handling_mfa(
            node, action, node.arn, condition_keys, service_control_policy_groups=scps
        )[0]:
            node.is_admin = True
            continue
        ...
        for attached_policy in node.attached_policies:
            if (
                attached_policy.arn != node.arn
                and ":aws:policy/" not in attached_policy.arn
            ):
                if query_interface.local_check_authorization_handling_mfa(
                    node,
...
def get_organizations_data(session: botocore.session.Session) -> OrganizationTree:
    ...
    try:
        ...
        if "AccessDeniedException" in str(ex):
            raise RuntimeError(
                ...
                "management account".format(
                    account_data["Arn"], account_data["Account"]
                )
            )
        else:
            raise ex
    ...
    logger.info(
        "Generating data for organization {} through management account {}".format(
    ...
    def _compose_ou(parent_id: str, parent_name: str) -> OrganizationNode:
        ...
        for ou_child_account_id in ou_child_account_list:
            ...
            org_account_objs.append(
                OrganizationAccount(
                    ou_child_account_id, child_account_scps, child_account_tags
                )
            )
        ...
        for child_ou_id in child_ou_ids:
            ...
            child_ous.append(
                _compose_ou(child_ou_id, desc_ou_resp["OrganizationalUnit"]["Name"])
            )

        return OrganizationNode(
            parent_id, parent_name, org_account_objs, child_ous, ou_scps, ou_tags
        )
